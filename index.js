...
fifaData.filter(function(item){
    if (item["Year"] === 2014 && item["Stage"] === "Final") {
        console.log(item["Home Team Name"] + "|" + item["Away Team Name"] + "|" + item["Away Team Goals"] + "|" + item["Win conditions"]);
    }
});
...
function getYears(finals) {
        // console.log(finals["Year"]);
    const gmetYears = finals.map(function(item){
        return item["Year"];
    });
    return gmetYears;
};
...
function getAverageGoals(data) {
    ...
    const totalAwayGoals = data.reduce(function(accum, item){
        return accum + item["Away Team Goals"];
    }, 0);
    
    console.log(`Average home team goals: ${Math.round(totalHomeGoals / data.length)} Average away team goals: ${Math.round(totalAwayGoals / data.length)}`);
};
